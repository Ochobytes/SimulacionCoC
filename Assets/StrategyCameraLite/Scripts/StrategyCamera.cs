...
public class StrategyCamera : MonoBehaviour
{
	...
	public Vector3 MinPositionMinZoom;
	public Vector3 MaxPositionMinZoom;
    public Vector3 MinPositionMaxZoom;
    public Vector3 MaxPositionMaxZoom;



    [Header("Zooming Camera Projection")]
	...
	private void OnDrawGizmosSelected()
	{
		...
		{
			var size = MaxPositionMinZoom - MinPositionMinZoom;
			var center = MinPositionMinZoom + size / 2;
			Gizmos.color = Color.cyan;
			Gizmos.DrawWireCube(center, size);

            size = MaxPositionMaxZoom - MinPositionMaxZoom;
            center = MinPositionMaxZoom + size / 2;
	...
	private void Movement()
	{
		...
		if (ConstrainPosition)
		{
			float t = Mathf.InverseLerp(MinZoomDistance, MaxZoomDistance, -currentZoom);
			targetPosition = Vector3.Min(Vector3.Max(targetPosition,
				Vector3.Lerp(MinPositionMinZoom, MinPositionMaxZoom, t)),
				Vector3.Lerp(MaxPositionMinZoom, MaxPositionMaxZoom, t));
        }
	}
	...
	{
		var localPos = !cameraOrthographic ? cam.transform.localPosition : new Vector3(0, 0, -cam.orthographicSize);
		...
		if (!cameraOrthographic)
			...
			cam.orthographicSize = -localPos.z;
	}
